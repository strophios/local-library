...
- **Guarantees**:
  ...
  - `clean_artifacts()` never raises; each pass catches exceptions and returns input unchanged
  - `clean_artifacts()` runs before `cleanup_markdown()` — artifact removal before formatting (pass ordering is load-bearing)
  - Non-Latin script filter targets CJK, Arabic, Thai, Devanagari; Greek and Cyrillic are exempt (legitimate in academic text)
  - Image description reformatting preserves substantive descriptions as `> [Figure]` blockquotes; removes noise (logos, seals, icons)
  - BOILERPLATE_RULES catalog is expected to grow as new publisher patterns are encountered
...
- **Library-scoped citekey resolution**: `CitekeyMapper.lookup()` and `CitekeyMapper.has_citekey()` require a keyword-only `library_id: int` parameter. This prevents cross-library citekey collisions (e.g., resolving a group library item when the personal library item was intended). `ZoteroReader.get_item()` similarly requires `library_id` and threads it through to the mapper.
- **Multi-library support**: ZoteroReader supports filtering by library via `library_id` parameter on relevant methods. `get_personal_library()` returns the user's personal library (type='user'). `list_citekeys_in_library(library_id)` returns citekeys scoped to a specific library. Collections include `library_id` to support library-scoped queries.
...
- **Artifact cleanup**: `clean_artifacts()` applies four composable passes (non-Latin script filtering → image description reformatting → watermark removal → publisher boilerplate stripping). Separate module from markdown_cleanup because it addresses content quality (removing things that shouldn't be there) rather than formatting. Runs before markdown cleanup in the pipeline. BOILERPLATE_RULES is a structured catalog (tuple of BoilerplateRule dataclasses) designed to grow as new publisher patterns are encountered.
...
- `CitekeyMapper.lookup()` and `CitekeyMapper.has_citekey()` require `library_id` as keyword-only argument — callers must always specify which library to resolve against. This is distinct from `LibraryJsonParser.has_citekey()` (JSON export backend) which has no library_id parameter.
- `ZoteroReader.get_item()` requires `library_id` as keyword-only argument — threads through to CitekeyMapper
- ZoteroReader methods that filter by collection/library (e.g., `list_citekeys_in_library`) require explicit library_id; personal library is typically libraryID=1 with type='user'
- ZoteroCollection.library_id links to Zotero's `libraries` table
