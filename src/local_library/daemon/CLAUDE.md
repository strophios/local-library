...
- `daemon start` immediately followed by `daemon stop` may hit the CLI's 10s stop timeout — SIGTERM is queued during warmup but only honored once the executor task returns. Acceptable: a warmup-window stop is rare in normal use, and the daemon does eventually exit cleanly. Set `LOCAL_LIBRARY_DAEMON_SKIP_WARMUP=1` if you need fast turnaround during development. This is one of four warts of the current "bind → warm → serve" startup sequence; do not fix in isolation. See `docs/feature-areas/neovim-citation-workflow/README.md` § "Daemon Startup Restructure" for the parallel-serve-and-warmup follow-up that resolves the whole class.
